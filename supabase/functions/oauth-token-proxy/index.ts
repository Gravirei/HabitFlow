...
function requireSecret(provider: Provider): string {
  const key = `${provider.toUpperCase().replace(/-/g, "_")}_CLIENT_SECRET`;
...
function resolveClientId(provider: Provider, fromRequest?: string): string {
  const key = `${provider.toUpperCase().replace(/-/g, "_")}_CLIENT_ID`;
  const configured = Deno.env.get(key);
  if (!configured && !fromRequest) {
    throw new Error(
      `${key} not configured and no clientId provided in request`,
    );
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!body.provider || !["google-calendar", "notion", "spotify", "slack"].includes(body.provider)) {
    ...
    if (
      ...
      ["google-calendar", "spotify"].includes(body.provider) &&
      !body.redirectUri
    ) {
