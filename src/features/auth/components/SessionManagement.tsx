...
import { signOutSession } from '@/lib/auth/api'
        ...
        <div className="dark:bg-card-dark rounded-lg bg-white p-4">
          <div className="flex items-start justify-between gap-4">
            ...
            <button
              ...
              onClick={() => load()}
              disabled={isLoading}
              className="rounded-full bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-700 disabled:opacity-50 dark:bg-slate-800 dark:text-slate-200"
            >
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-sm font-semibold text-slate-900 dark:text-white">
                        {s.device_info?.browser || 'Device'}{' '}
                        {s.device_info?.os ? `(${s.device_info.os})` : ''}
                      </p>
                    ...
                    <button
                      type="button"
                      className="rounded-full bg-red-50 px-3 py-1.5 text-xs font-semibold text-red-600 dark:bg-red-900/20 dark:text-red-400"
                      onClick={async () => {
                        const isCurrentSession =
                          currentSessionToken !== null && s.session_token === currentSessionToken
          ...
          <div className="mt-4">
            <button
              ...
              className="w-full rounded-full bg-red-50 px-4 py-2 text-sm font-semibold text-red-600 disabled:opacity-50 dark:bg-red-900/20 dark:text-red-400"
              onClick={async () => {
                const confirmed = confirm(
                  'Log out of all devices? You will also be signed out here.'
                )
                if (!confirmed) return
