...
export function NudgeMessageCard({ payload, senderName }: NudgeMessageCardProps) {
  ...
  return (
    <motion.div
      {...anim}
      className="w-full max-w-[300px] rounded-[16px] border border-amber-300/20 bg-amber-300/[0.07] px-4 py-3 shadow-[0_18px_50px_rgba(0,0,0,0.3)] backdrop-blur-xl transition-all duration-200 hover:-translate-y-0.5 hover:bg-amber-300/[0.11]"
      role="article"
      aria-label={`Nudge from ${senderName}`}
    >
      <div className="flex items-start gap-3">
        <div className="flex size-10 flex-shrink-0 items-center justify-center rounded-2xl border border-amber-300/25 bg-amber-300/[0.12]">
          <span
