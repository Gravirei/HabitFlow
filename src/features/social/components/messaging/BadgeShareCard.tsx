...
export function BadgeShareCard({ payload }: BadgeShareCardProps) {
  ...
  return (
    <motion.div
      {...anim}
      className="group w-full max-w-[310px] overflow-hidden rounded-[18px] border border-white/[0.08] bg-white/[0.025] shadow-[0_18px_50px_rgba(0,0,0,0.35)] backdrop-blur-xl transition-all duration-200 hover:-translate-y-0.5 hover:border-white/[0.12]"
      role="article"
      aria-label={`Badge: ${payload.badgeName}`}
    >
      <div
        className={`flex items-center gap-2 border-b border-white/[0.08] bg-gradient-to-br ${theme.topBg} px-4 py-3`}
      >
        <span
        ...
        <span className={`text-[10px] font-extrabold uppercase tracking-[0.14em] ${theme.accent}`}>
          {payload.isLevelUp ? 'Level up' : 'Badge unlocked'}
        </span>
        <span
          className={`ml-auto rounded-full border px-2.5 py-1 text-[9px] font-extrabold uppercase tracking-[0.14em] ${theme.pill}`}
        >
          {payload.badgeRarity}
        </span>
      </div>
      ...
      <div className="px-4 py-4 text-center">
        <div className="relative mx-auto">
          <div
            className={`relative mx-auto flex size-[62px] items-center justify-center rounded-[18px] bg-gradient-to-br ${theme.iconBg} overflow-hidden shadow-[0_18px_50px_rgba(0,0,0,0.35)]`}
          >
            {!reduced && (
              <div
        ...
        <div className="mt-3 text-[15px] font-extrabold tracking-tight text-white">
          {payload.badgeName}
        </div>
      ...
      <div className="border-t border-white/[0.08] px-4 py-3">
        <button
          type="button"
          className="w-full cursor-pointer rounded-xl border border-white/[0.10] bg-white/[0.03] py-2 text-[12px] font-semibold text-white/70 transition-colors hover:bg-white/[0.06] hover:text-white"
          aria-label="React to badge"
        >
          <span
            className="material-symbols-outlined mr-2 align-[-2px] text-[16px]"
            style={{ fontVariationSettings: "'FILL' 1" }}
          >
            mood
          </span>
