...
function XPEventRow({
  ...
  event: { label: string; icon: string; total: number; count: number }
  index: number
}) {
  return (
    <motion.div
      ...
      className="flex items-center gap-3 rounded-xl border border-white/[0.04] bg-white/[0.03] px-3 py-2.5"
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-[12px] font-medium text-white">{event.label}</p>
        {event.count > 1 && <p className="text-[10px] text-slate-500">×{event.count}</p>}
      </div>
      <span className="text-[13px] font-bold tabular-nums text-emerald-400">+{event.total}</span>
    </motion.div>
  )
}
...
function XPBreakdown({ events, todayXP }: { events: XPEvent[]; todayXP: number }) {
  const grouped = events.reduce<
    Record<string, { label: string; icon: string; total: number; count: number }>
  >((acc, e) => {
    ...
    <div className="rounded-2xl border border-white/[0.04] bg-white/[0.015] p-4">
      <div className="mb-3 flex items-center justify-between">
        ...
        {todayXP > 0 && (
          <div className="flex items-center gap-1 rounded-full bg-primary/10 px-2.5 py-1">
            <span className="text-[12px] font-bold tabular-nums text-primary">+{todayXP}</span>
      ...
      {sorted.length === 0 ? (
        <div className="flex flex-col items-center gap-2 py-6 text-center">
          ...
          <p className="text-[12px] font-medium text-slate-500">No XP earned yet today</p>
...
export function ProfileTab() {
  ...
  return (
    <div className="space-y-5">
      {/* Profile hero */}
      <div className="relative overflow-hidden rounded-2xl border border-white/[0.05] bg-gradient-to-br from-slate-800 via-slate-800/95 to-slate-900 p-5">
        <div
          className="pointer-events-none absolute -right-16 -top-16 size-36 rounded-full blur-3xl"
          style={{ backgroundColor: league.color + '12' }}
        />
        ...
        <div className="relative flex items-center gap-4">
          <div className="relative flex-shrink-0">
            ...
            <img
              src={displayAvatar}
              alt={fullName}
              className="relative size-14 rounded-2xl object-cover ring-2 ring-slate-800"
            />
            <div
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11px] font-medium text-slate-400">{league.label}</p>
          </div>
          <button
            onClick={() => setShowSummary(true)}
            className="flex cursor-pointer items-center gap-1.5 rounded-xl bg-primary/10 px-3 py-2 text-[11px] font-semibold text-primary transition-colors duration-200 hover:bg-primary/20"
          >
      ...
      <button
        ...
        className="flex w-full cursor-pointer items-center justify-center gap-2 rounded-2xl border border-white/[0.04] bg-white/[0.02] py-3 text-[12px] font-semibold text-slate-400 transition-all duration-200 hover:bg-white/[0.04] hover:text-white"
      >
        <span className="material-symbols-outlined text-sm">summarize</span>
        See Today's Summary
      </button>
      ...
      <div>
        <div className="mb-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="flex h-5 items-center rounded-md border border-white/[0.06] bg-white/[0.04] px-1.5 text-[10px] font-bold tabular-nums text-primary">
              {unlocked.length}/{unlocked.length + locked.length}
            </span>
          </div>
          <div className="flex gap-0.5 rounded-lg border border-white/[0.04] bg-white/[0.02] p-0.5">
            {(['all', 'unlocked', 'locked'] as const).map((f) => (
              <button
                key={f}
                onClick={() => setBadgeFilter(f)}
                className={`cursor-pointer rounded-md px-2.5 py-1 text-[10px] font-semibold transition-all duration-200 ${
                  badgeFilter === f
        ...
        {badges.length > 0 ? (
          ...
          <div className="flex flex-col items-center gap-3 py-12 text-center">
            ...
            <p className="text-[12px] font-medium text-slate-400">No badges to show</p>
          </div>
        )}
      </div>
