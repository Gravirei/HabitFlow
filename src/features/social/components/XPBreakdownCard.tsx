    ...
    return (
      <div className="rounded-2xl border border-slate-700/50 bg-slate-800/50 p-6">
        ...
        <div className="flex flex-col items-center justify-center py-6 text-center">
          <div className="mb-3 flex size-14 items-center justify-center rounded-2xl bg-slate-700/50">
            <span className="material-symbols-outlined text-3xl text-slate-500">electric_bolt</span>
          </div>
          <p className="text-sm font-medium text-slate-400">No XP earned yet today</p>
          <p className="mt-1 text-xs text-slate-500">Complete a habit to start earning!</p>
        </div>
      </div>
    )
  }
        ...
        {sortedGroups.map(([type, data], index) => (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              {data.count > 1 && <p className="text-[11px] text-slate-400">×{data.count}</p>}
            </div>
