...
function RankBadge({ entry }: { entry: LeaderboardEntry }) {
  if (entry.rankChange === 'new') {
    return (
      <span className="inline-flex items-center rounded-md bg-cyan-500/15 px-1.5 py-0.5 text-[9px] font-extrabold uppercase tracking-wider text-cyan-400">
        New
      </span>
    )
  }
...
function Podium({ entries }: { entries: LeaderboardEntry[] }) {
  ...
  const medalShadows = ['shadow-gray-400/20', 'shadow-yellow-500/30', 'shadow-amber-700/20']
  return (
    <div className="flex items-end justify-center gap-3 pb-2 pt-4">
      {podiumOrder.map((entry, i) => (
        ...
        >
          ...
          <div className="relative mb-2">
            {i === 1 && (
              <motion.span
                ...
                className="material-symbols-outlined absolute -top-5 left-1/2 -translate-x-1/2 text-xl text-yellow-400"
            ...
            <div className={`relative ${i === 1 ? 'size-16' : 'size-12'}`}>
              <div
                className={`absolute -inset-[2px] rounded-xl bg-gradient-to-br ${medalColors[i]} ${medalShadows[i]} shadow-lg`}
              />
              <img
                src={entry.avatarUrl}
            ...
            {/* Rank number */}
            <div
              className={`absolute -bottom-1 -right-1 flex size-5 items-center justify-center rounded-md bg-gradient-to-br ${medalColors[i]} shadow-md`}
            >
          ...
          {/* Name */}
          <p
            className={`max-w-[72px] truncate text-center text-[11px] font-semibold text-white ${
          ...
          {/* XP */}
          <p className="mt-0.5 text-[10px] font-bold text-slate-400">{entry.xp} XP</p>
          ...
          <motion.div
            ...
            className={`${heights[i]} mt-2 flex w-16 items-start justify-center rounded-t-xl border border-b-0 border-white/[0.04] bg-gradient-to-t from-slate-800/80 to-slate-700/40 pt-1.5`}
          >
  ...
  return (
    ...
    >
      ...
      <div className="flex size-8 flex-shrink-0 items-center justify-center">
        <span
          className={`text-sm font-bold tabular-nums ${entry.isCurrentUser ? 'text-primary' : 'text-slate-500'}`}
        >
          {entry.rank}
        </span>
      </div>
      ...
      <div className="min-w-0 flex-1">
        <p
          className={`truncate text-[13px] font-semibold ${entry.isCurrentUser ? 'text-primary' : 'text-white'}`}
        >
          {entry.isCurrentUser ? 'You' : entry.displayName}
        </p>
        <p className="text-[10px] font-medium text-slate-500">Level {entry.level}</p>
      </div>
      ...
      {!entry.isCurrentUser && (
        <div className="mr-1 flex-shrink-0">
          {isFriend ? (
            <span className="flex items-center gap-0.5 rounded-lg bg-emerald-400/10 px-2 py-1 text-[10px] font-medium text-emerald-400">
              <span
            ...
            <span className="flex items-center gap-0.5 rounded-lg bg-amber-400/10 px-2 py-1 text-[10px] font-medium text-amber-400">
            ...
            <button
              ...
              className="flex cursor-pointer items-center gap-0.5 rounded-lg bg-primary/10 px-2 py-1 text-[10px] font-semibold text-primary transition-colors hover:bg-primary/20"
            >
...
export function LeaderboardScreen() {
  ...
  return (
    <div className="space-y-4">
      {/* Period Toggle */}
      <div className="flex gap-1 rounded-xl border border-white/[0.04] bg-slate-800/50 p-1">
        {periods.map((p) => (
          <button
            key={p.id}
      ...
      {isLoading ? (
        <div className="space-y-3 pt-8">
          ...
          <div className="flex items-end justify-center gap-3 pb-4">
            {[56, 72, 48].map((h, i) => (
              <div key={i} className="flex animate-pulse flex-col items-center">
                ...
                <div className="mt-2 h-2 w-12 rounded bg-slate-700/40" />
                <div className={`mt-2 w-16 rounded-t-xl bg-slate-800/40`} style={{ height: h }} />
              </div>
            ))}
          </div>
          {Array.from({ length: 5 }).map((_, i) => (
            <div
              key={i}
              className="flex animate-pulse items-center gap-3 rounded-2xl bg-slate-800/20 px-3.5 py-3"
            >
        ...
        <div className="flex flex-col items-center justify-center gap-4 py-20 text-center">
          ...
          <div>
            ...
            <p className="mx-auto mt-1 max-w-[220px] text-xs text-slate-500">
              Complete habits to earn XP and start climbing!
            </p>
          </div>
        </div>
      ) : (
      ...
      <ProfilePreviewModal
        ...
        showAddFriend={
          previewProfile
            ? !friendUserIds.has(previewProfile.userId) &&
              !pendingRequestUserIds.has(previewProfile.userId)
            : false
        }
        onAddFriend={(userId) => {
