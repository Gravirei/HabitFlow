...
export const useIntervals = (options?: UseIntervalsOptions): UseIntervalsReturn => {
  ...
  const handleIntervalComplete = useCallback(
    (now: number, _elapsed: number): boolean => {
      // Prevent concurrent transitions (race condition guard)
      if (transitionLockRef.current) {
        logger.warn('Transition already in progress, skipping completion check', {
          context: 'useIntervals',
        })
        return false
      }
  ...
  const handleIntervalSwitch = useCallback(
    (now: number) => {
      // Prevent concurrent transitions (race condition guard)
      if (transitionLockRef.current) {
        logger.warn('Transition already in progress, skipping interval switch', {
          context: 'useIntervals',
        })
        return
      }
  ...
  useEffect(() => {
    ...
    const currentDuration =
      currentInterval === 'work' ? workMinutes * MS_PER_MINUTE : breakMinutes * MS_PER_MINUTE
    ...
    if (sessionStartTime !== null) {
      if (intervalStartTime !== null) {
        ...
        const pauseStartTime =
          sessionStartTime + (now - sessionStartTime - totalPausedTime - basePausedElapsed)
  ...
  const restoreTimer = useCallback(
    (state: any) => {
      ...
      if (state.isPaused) {
        ...
        const elapsed =
          (state.intervalStartTime ? now - state.intervalStartTime : 0) + state.pausedElapsed
        ...
        if (remaining > 0) {
          ...
          setSessionStartTime(
            now - (elapsed + state.currentLoop * (state.workDuration + state.breakDuration))
          )
        } else {
