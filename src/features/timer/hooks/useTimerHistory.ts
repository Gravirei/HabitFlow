    ...
    (options: SaveHistoryOptions) => {
      ...
      try {
        ...
        if (duration <= 0 || isNaN(duration) || !isFinite(duration)) {
          logger.error('Invalid duration', undefined, {
            context: 'useTimerHistory',
            data: { duration },
          })
          logError(
            new Error('Invalid duration for history record'),
        ...
        if (
          metadata.intervalCount !== undefined &&
          (metadata.intervalCount < 0 ||
            isNaN(metadata.intervalCount) ||
            !isFinite(metadata.intervalCount))
        ) {
          logger.error('Invalid interval count', undefined, {
            context: 'useTimerHistory',
            data: { intervalCount: metadata.intervalCount },
          })
          logError(
            new Error('Invalid interval count for history record'),
            'useTimerHistory.saveToHistory'
          )
          return
        }
        ...
        logger.debug('Created record', {
          context: 'useTimerHistory',
          data: { id: record.id, mode: record.mode, duration: record.duration },
        })
