...
export const TimerDisplay: React.FC<TimerDisplayProps> = React.memo(
  ({
    ...
    const isLastThirtySeconds =
      (mode === 'Countdown' || mode === 'Intervals') && timeLeft <= 30000 && timeLeft > 0

    // Check if we're in the last 10 seconds (for extra urgency with pulse animation)
    const isLastTenSeconds =
      (mode === 'Countdown' || mode === 'Intervals') && timeLeft <= 10000 && timeLeft > 0
      ...
      <>
        ...
        {showIntervalStatus && currentInterval && (
          <div className={TIMER_CLASSES.interval.container} role="status" aria-live="polite">
            <p className={TIMER_CLASSES.interval.count}>Interval {intervalCount + 1}</p>
        ...
        <div className={TIMER_CLASSES.timerRing.container}>
          <div
            className={`${TIMER_CLASSES.timerRing.wrapper} ${isLastTenSeconds && !prefersReducedMotion ? 'animate-pulse' : ''}`}
          >
            <svg
