...
describe('useAchievementsStore', () => {
  ...
  describe('unlockAchievement', () => {
    ...
    it('should not re-unlock already unlocked achievement', () => {
      ...
      const firstUnlockTime = useAchievementsStore
        .getState()
        .getAchievementById(achievementId)?.unlockedAt

      act(() => {
        unlockAchievement(achievementId)
      })

      const secondUnlockTime = useAchievementsStore
        .getState()
        .getAchievementById(achievementId)?.unlockedAt
  ...
  describe('updateAchievements', () => {
    ...
    it('should update progress based on user stats', () => {
      ...
      const timeAchievement = useAchievementsStore
        .getState()
        .achievements.find((a) => a.category === 'time')
    ...
    it('should auto-unlock achievement when requirement is met', () => {
      ...
      const achievement = useAchievementsStore
        .getState()
        .achievements.find((a) => a.id === 'time_10h')
      expect(achievement?.unlocked).toBe(true)
      expect(achievement?.unlockedAt).toBeInstanceOf(Date)
    })
    ...
    it('should not modify already unlocked achievements', () => {
      ...
      const unlockedAt = useAchievementsStore
        .getState()
        .getAchievementById(achievementId)?.unlockedAt
    ...
    it('should update session-based achievements', () => {
      ...
      const sessionAchievement = useAchievementsStore
        .getState()
        .achievements.find((a) => a.id === 'sessions_50')
