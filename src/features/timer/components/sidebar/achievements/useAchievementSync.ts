...
export function useAchievementSync() {
  ...
  const normalizeHistory = useCallback(
    (history: TimerHistoryRecord[] | unknown): TimerHistoryRecord[] => {
   ...
   */
  const convertToSessions = useCallback(
    (records: TimerHistoryRecord[]) => {
  ...
  const checkSpecialAchievements = useCallback(
    (allRecords: TimerHistoryRecord[]) => {
      ...
      for (let i = 1; i < sortedRecords.length; i++) {
        const gap =
          (sortedRecords[i].timestamp - sortedRecords[i - 1].timestamp) / (1000 * 60 * 60 * 24)
        if (gap >= 30) {
  ...
  const syncAchievements = useCallback(() => {
    ...
    const allRecords = [...stopwatchHistory, ...countdownHistory, ...intervalsHistory]
