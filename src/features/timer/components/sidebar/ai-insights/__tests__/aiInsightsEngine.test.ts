...
describe('aiInsightsEngine', () => {
  ...
  describe('analyzeModeMastery', () => {
    ...
    it('should handle modes with no sessions', () => {
      const sessions = Array.from({ length: 10 }, () => createMockSession({ mode: 'Countdown' }))
  ...
  describe('analyzeConsistency', () => {
    ...
    it('should track longest streak metric', () => {
      ...
      const sessions = [createSessionOnDate(0), createSessionOnDate(1), createSessionOnDate(5)]
    ...
    it('should detect improving trend', () => {
      // Old: few sessions
      const oldSessions = Array.from({ length: 3 }, (_, i) => createSessionOnDate(20 - i))
      // Recent: many sessions
      const recentSessions = Array.from({ length: 10 }, (_, i) => createSessionOnDate(6 - i))
    ...
    it('should detect declining trend', () => {
      ...
      const oldSessions = Array.from({ length: 14 }, (_, i) => createSessionOnDate(20 - i))
  ...
  describe('analyzeProductivityTrend', () => {
    ...
    it('should calculate completion rates for both periods', () => {
      ...
      const currentWeek = [
        ...
        createMockSession({
          completed: true,
          startTime: new Date(now - dayMs),
          endTime: new Date(now - dayMs + 1000),
        }),
        createMockSession({
          completed: false,
          startTime: new Date(now - 2 * dayMs),
          endTime: new Date(now - 2 * dayMs + 1000),
        }),
        createMockSession({
          completed: false,
          startTime: new Date(now - 3 * dayMs),
          endTime: new Date(now - 3 * dayMs + 1000),
        }),
      ]
  ...
  describe('calculateProductivityScore', () => {
    ...
    it('should give high duration score for optimal duration (15-45 min)', () => {
      ...
      const sessions = Array.from({ length: 10 }, () => createMockSession({ duration: 1500 }))
    ...
    it('should give lower duration score for very short sessions', () => {
      ...
      const sessions = Array.from({ length: 10 }, () => createMockSession({ duration: 300 }))
  ...
  describe('generateWeeklySummary', () => {
    ...
    it('should only include sessions from the last 7 days', () => {
      ...
      const oldSessions = [
        createMockSession({
          startTime: new Date(now - 10 * dayMs),
          endTime: new Date(now - 10 * dayMs + 1000),
        }),
      ]
    ...
    it('should identify most productive day', () => {
      ...
      const day2 = [
        createMockSession({
          duration: 5000,
          startTime: new Date(now - dayMs),
          endTime: new Date(now - dayMs + 5000000),
        }),
      ]
    ...
    it('should set correct period dates', () => {
      const now = new Date()
      const sessions = [
        createMockSession({ startTime: now, endTime: new Date(now.getTime() + 1000) }),
      ]
