...
export function generateAIInsights(sessions: TimerSessionData[]): AIInsights {
  ...
  const dataRange = {
    start: sortedSessions.length > 0 ? new Date(sortedSessions[0].startTime) : now,
      ...
      sortedSessions.length > 0 ? new Date(sortedSessions[sortedSessions.length - 1].endTime) : now,
...
export function analyzeConsistency(sessions: TimerSessionData[]): ConsistencyInsight {
  ...
  if (dateKeys.includes(today) || dateKeys.includes(yesterday)) {
    currentStreak = 1
    const checkDate = dateKeys.includes(today)
      ? new Date()
  ...
  activeDaysSet.forEach((dateKey) => {
    const count = sortedSessions.filter(
      (s) => new Date(s.startTime).toDateString() === dateKey
    ).length
  ...
  const variance =
    dailySessionCounts.reduce((sum, c) => sum + Math.pow(c - avgSessions, 2), 0) /
    dailySessionCounts.length
  ...
  const trend =
    recentAvg > olderAvg * 1.2 ? 'improving' : recentAvg < olderAvg * 0.8 ? 'declining' : 'stable'
...
export function analyzeProductivityTrend(sessions: TimerSessionData[]): ProductivityTrendInsight {
  ...
  const currentWeekSessions = sessions.filter(
    (s) => new Date(s.startTime).getTime() >= now - weekMs
  )
  ...
  const previous = {
    ...
    completionRate:
      previousWeekSessions.length > 0
        ? (previousWeekSessions.filter((s) => s.completed).length / previousWeekSessions.length) *
          100
        : 0,
  }
...
export function calculateProductivityScore(sessions: TimerSessionData[]): ProductivityScore {
  ...
  const improvementScore = Math.min(
    100,
    Math.max(0, 50 + (recentCompletion - olderCompletion) * 100)
  )
...
export function generateWeeklySummary(sessions: TimerSessionData[]): WeeklySummary {
  ...
  const completionRate =
    (weeklySessions.filter((s) => s.completed).length / weeklySessions.length) * 100
  ...
  const longest = weeklySessions.reduce(
